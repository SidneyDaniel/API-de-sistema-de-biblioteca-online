...
function verificar(req: Request, res: Response, next: NextFunction):void {
...
function verificarAdm(req:Request, res:Response, next:NextFunction): void {
    const cookieHeader = req.headers.cookie || '';
    const cookies = cookieHeader.split(';').map(cookie => cookie.trim());
    const sessionCookie = cookies.find(cookie => cookie.startsWith('session='));
    const sessionValue = sessionCookie ? sessionCookie.split('=')[1] : '';
...
export default { verificar, verificarAdm }